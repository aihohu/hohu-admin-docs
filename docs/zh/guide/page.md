...
| 操作            | 说明                    | 示例                        |
| --------------- | ----------------------- | --------------------------- |
| `"contains"`    | 模糊匹配 `LIKE '%val%'` | `user_name` 字段搜索        |
| `"ilike"`       | 不区分大小写模糊匹配    | `ILIKE '%val%'`             |
| `"like"`        | LIKE 自定义模式         | value 中包含 `%` 通配符     |
| `"startswith"`  | 前缀匹配 `LIKE 'val%'`  | 以指定前缀开头              |
| `"endswith"`    | 后缀匹配 `LIKE '%val'`  | 以指定后缀结尾              |
| `"=="`          | 精确匹配                | `status` 字段筛选           |
| `"!="`          | 不等于                  | 排除指定值                  |
| `">"`           | 大于                    | 数值比较                    |
| `"<"`           | 小于                    | 数值比较                    |
...
| `"<="`          | 小于等于                | 日期区间终点                |
| `"in_"`         | IN 查询                 | 批量 ID 查询                |
| `"not_in"`      | NOT IN 查询             | 排除多个值                  |
| `"between"`     | BETWEEN 范围查询        | value 传 `[a, b]` 元组/列表 |
| `"is_null"`     | IS NULL 判断            | value 为 `True` 时判断为空  |
| `"is_not_null"` | IS NOT NULL 判断        | value 为 `True` 时判断非空  |
